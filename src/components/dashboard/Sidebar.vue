<template>
  <aside class="w-64 bg-white rounded-lg p-4 shadow h-full">
    <div class="mb-6">
      <div class="inline-flex items-center justify-center w-12 h-12 bg-indigo-50 rounded-md text-indigo-600 font-bold">SH</div>
    </div>
    <nav class="space-y-2">
      ...
      <router-link to="/routine/add-period" class="block px-3 py-2 rounded hover:bg-indigo-50 text-gray-700">Add Period</router-link>
...
export default {
  ...
  data() {
    return {
      username: localStorage.getItem('rememberUsername') || localStorage.getItem('username') || ''
    }
  }
};
</script>
